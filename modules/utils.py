...
def cosine_similarity_along_F(feature_field, external_feats):
    """
    Compute cosine similarity between each pixel in feature_field (BxNxF)
    and the external feature vector (BxF), returning a similarity score per pixel.

    Args:
        feature_field (torch.Tensor): Shape (B, N, F)
        external_feats (torch.Tensor): Shape (B, F)

    Returns:
        torch.Tensor: Cosine similarity scores of shape (B, N)
    """
    # Normalize feature_field along feature dimension F
    feature_field_norm = F.normalize(feature_field, p=2, dim=-1)  # (B, N, F)

    # Normalize external_feats along feature dimension F and expand for broadcasting
    external_feats_norm = F.normalize(external_feats, p=2, dim=-1).unsqueeze(1)  # (B, 1, F)

    # Compute dot product along feature dimension F to get cosine similarity (B, N)
    cosine_sim = torch.sum(feature_field_norm * external_feats_norm, dim=-1)  # (B, N)
...
def compute_iou_batch(preds, gt):
    intersections = torch.logical_and(preds, gt)
    unions = torch.logical_or(preds, gt)

    intersections_sum = intersections.sum(dim=(1, 2))
    unions_sum = unions.sum(dim=(1, 2))
...
def get_features_in_mask(features, mask):
    _, w = mask.shape
    mask_idx_row, mask_idx_col = torch.where(mask)
    mask_idx = mask_idx_row * w + mask_idx_col

    features_reshape = torch.reshape(features, [-1, features.shape[-1]])
    features_in_mask = features_reshape[mask_idx]

    return features_in_mask, mask_idx_row, mask_idx_col


def set_features_in_mask(features, masks):
    # sets the features at the True entries of masks
    # features is of size BxNXF
    # masks is of size BxHxW
    # The number of True elements in each mask equals N
    # The output of the function is BxHxWXF (B: batch size, H: height of the mask, W: width of the mask, F: feature size)
    
    # sizes
    f = features.shape[-1]
    b, h, w = masks.shape

    features_in_mask = torch.zeros([b, h, w, f], dtype=features.dtype, device=features.device)
    features_in_mask[masks, :] = features.reshape(-1, f)

    return features_in_mask
